...
public interface ContractService {
    ...
    APIResult<Contracts> list(
            ...
            @ApiParam(value = "合同状态: 1.已驳回 2.审核中 3.未生效 4.已生效 (JsonArray<Integer>)") @RequestParam(value = "status", required = false) String status,
            @ApiParam(value = "合同模版: 为空则查询全部 (JsonArray<Long>)") @RequestParam(value = "templateid", required = false) String templateid,
            @ApiParam(value = "合同类型: 1.主合同 2.附加合同 (JsonArray<Integer>)") @RequestParam(value = "type", required = false) String type,
